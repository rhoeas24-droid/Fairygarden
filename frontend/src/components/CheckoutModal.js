...
import {
  X, Loader2, MapPin, User, Mail, Phone,
  FileText, Truck, Check, ChevronRight,
  ChevronLeft, ShieldCheck, LogIn, UserPlus, ShoppingBag
} from 'lucide-react';
...
const AuthPreStep = ({ t, onSignIn, onRegister, onGuest }) => (
  <div className="space-y-4 py-2" data-testid="checkout-step-auth">
    ...
    <button onClick={onSignIn}
      className="w-full flex items-center gap-4 p-4 bg-forest/40 border border-gold/20 rounded-xl hover:border-gold/40 hover:bg-gold/5 transition-all group"
...
const CheckoutModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async () => {
    if (!canSubmit) { toast.error(t('checkout.acceptRequired', 'Please accept Terms & Conditions and Privacy Policy.')); return; }
        ...
        {isOpen && (
          <React.Fragment>
            ...
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
              className="fixed top-[5vh] left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-2xl max-h-[90vh] bg-forest-dark border border-gold/30 rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden"
              ...
              <div className="p-6 space-y-4 overflow-y-auto">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-cinzel text-gold font-bold mb-2">{t('checkout.orderNumber', 'Order')} #{orderResult.order_id}</h3>
                ...
                <div className="bg-forest/40 border border-gold/15 rounded-xl p-4 space-y-3">
                  ...
                  <div className="bg-forest/60 rounded-lg p-3 border border-gold/10 mt-3">
                    <p className="text-cream/60 font-montserrat text-xs">{t('checkout.orderRef', 'Please use your order number as payment reference')}: <span className="text-gold font-bold">#{orderResult.order_id}</span></p>
                  </div>
                </div>
                <GoldButton onClick={onClose} className="w-full" dataTestId="bank-transfer-close">{t('checkout.close', 'Close')}</GoldButton>
              </div>
            </motion.div>
          </React.Fragment>
        )}
  ...
  return (
        ...
        <React.Fragment>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
            ...
            <div className="p-5 border-b border-gold/20 flex items-center justify-between shrink-0">
              <h2 className="text-xl sm:text-2xl font-cinzel font-bold text-gold" data-testid="checkout-title">{t('checkout.title', 'Checkout')}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {step === -1 && (
                <AuthPreStep t={t}
                  ...
                  onRegister={() => { setShowAuthModal(true); setAuthMode('register'); }}
                  onGuest={() => setStep(0)} />
              )}
                ...
                <div className="space-y-5" data-testid="checkout-step-address">
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <InputField icon={User} label={t('checkout.firstName', 'First Name') + ' *'} testId="checkout-first-name" value={form.billing_first_name} onChange={update('billing_first_name')} required />
                      <InputField icon={User} label={t('checkout.lastName', 'Last Name') + ' *'} testId="checkout-last-name" value={form.billing_last_name} onChange={update('billing_last_name')} required />
                      <InputField icon={Mail} label={t('checkout.email', 'Email') + ' *'} testId="checkout-email" type="email" value={form.billing_email} onChange={update('billing_email')} required />
                      <InputField icon={Phone} label={t('checkout.phone', 'Phone')} testId="checkout-phone" type="tel" value={form.billing_phone} onChange={update('billing_phone')} />
                    </div>
                  </div>
                  <div>
                    <InputField icon={MapPin} label={t('checkout.address', 'Address') + ' *'} testId="checkout-address" value={form.billing_address_1} onChange={update('billing_address_1')} required />
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
                      <InputField icon={MapPin} label={t('checkout.city', 'City') + ' *'} testId="checkout-city" value={form.billing_city} onChange={update('billing_city')} required />
                      <InputField icon={MapPin} label={t('checkout.postcode', 'Postcode') + ' *'} testId="checkout-postcode" value={form.billing_postcode} onChange={update('billing_postcode')} required />
                      <InputField icon={MapPin} label={t('checkout.country', 'Country') + ' *'} testId="checkout-country" value={form.billing_country} onChange={update('billing_country')} required />
                    </div>
                  </div>
                  ...
                  {!form.same_as_billing && (
                    <div>
                      <h3 className="font-cinzel text-gold text-sm font-bold mb-3 uppercase tracking-wider flex items-center gap-2"><Truck className="w-4 h-4" />{t('checkout.shippingAddress', 'Shipping Address')}</h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                        <InputField icon={User} label={t('checkout.firstName', 'First Name')} testId="checkout-ship-first" value={form.shipping_first_name} onChange={update('shipping_first_name')} />
                        <InputField icon={User} label={t('checkout.lastName', 'Last Name')} testId="checkout-ship-last" value={form.shipping_last_name} onChange={update('shipping_last_name')} />
                      </div>
                      <InputField icon={MapPin} label={t('checkout.address', 'Address')} testId="checkout-ship-address" value={form.shipping_address_1} onChange={update('shipping_address_1')} />
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
                        <InputField label={t('checkout.city', 'City')} testId="checkout-ship-city" value={form.shipping_city} onChange={update('shipping_city')} />
                        <InputField label={t('checkout.postcode', 'Postcode')} testId="checkout-ship-postcode" value={form.shipping_postcode} onChange={update('shipping_postcode')} />
                        <InputField label={t('checkout.country', 'Country')} testId="checkout-ship-country" value={form.shipping_country} onChange={update('shipping_country')} />
                      </div>
                    </div>
                  )}
                </div>
              ...
              {step === 1 && (
                <div className="space-y-4" data-testid="checkout-step-shipping">
                  <h3 className="font-cinzel text-gold text-sm font-bold uppercase tracking-wider flex items-center gap-2"><Truck className="w-4 h-4" />{t('checkout.selectShipping', 'Select Shipping Method')}</h3>
                  ...
                  <div className="mt-4">
                    <h3 className="font-cinzel text-gold text-sm font-bold mb-2 uppercase tracking-wider flex items-center gap-2"><FileText className="w-4 h-4" />{t('checkout.notes', 'Order Notes')}</h3>
                    <textarea data-testid="checkout-notes" value={form.order_notes} onChange={update('order_notes')} rows={3} placeholder={t('checkout.notesPlaceholder', 'Any special instructions...')}
              ...
              {step === 2 && (
                <div className="space-y-4" data-testid="checkout-step-summary">
                  <div className="bg-forest/40 border border-gold/15 rounded-xl p-4">
                    <h3 className="font-cinzel text-gold text-sm font-bold mb-3 uppercase tracking-wider">{t('checkout.orderSummary', 'Order Summary')}</h3>
                    ...
                    <div className="mt-2 pt-2 border-t border-gold/10 space-y-1">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-cream font-montserrat">{shippingCost === 0 ? t('checkout.free', 'Free') : `\u20AC${shippingCost.toFixed(2)}`}</span>
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="bg-forest/40 border border-gold/15 rounded-xl p-3">
                      ...
                      <p className="text-cream/70 font-montserrat text-xs leading-relaxed">
                        {form.same_as_billing
                          ? <React.Fragment>{form.billing_first_name} {form.billing_last_name}<br />{form.billing_address_1}<br />{form.billing_city}, {form.billing_postcode}<br />{form.billing_country}</React.Fragment>
                          : <React.Fragment>{form.shipping_first_name} {form.shipping_last_name}<br />{form.shipping_address_1}<br />{form.shipping_city}, {form.shipping_postcode}<br />{form.shipping_country}</React.Fragment>
                        }
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    <h3 className="font-cinzel text-gold text-sm font-bold mb-2 uppercase tracking-wider">{t('checkout.paymentMethod', 'Payment Method')}</h3>
                    <div className="space-y-2">
                  ...
                  <div className="space-y-3 pt-2">
                    <label className="flex items-start gap-3 cursor-pointer" data-testid="accept-terms">
                      ...
                      <span className="text-cream/80 font-montserrat text-xs">
                        ...
                        <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('openTermsModal'))} className="text-gold underline hover:text-gold-light">{t('checkout.termsConditions', 'Terms & Conditions')}</button> *
                      </span>
                    </label>
                    <label className="flex items-start gap-3 cursor-pointer" data-testid="accept-privacy">
                      ...
                      <span className="text-cream/80 font-montserrat text-xs">
                        {t('checkout.iAccept', 'I accept the')}{' '}
                        <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('openPrivacyModal'))} className="text-gold underline hover:text-gold-light">{t('checkout.privacyPolicy', 'Privacy Policy')}</button> *
                      </span>
                    </label>
                    <label className="flex items-start gap-3 cursor-pointer" data-testid="subscribe-newsletter">
                      ...
                      <span className="text-cream/80 font-montserrat text-xs">{t('checkout.subscribeNewsletter', 'Subscribe to our newsletter for magical updates')}</span>
                    </label>
                  </div>
                </div>
              )}
            ...
            {step >= 0 && (
              <div className="p-4 border-t border-gold/20 shrink-0 flex items-center gap-3">
                {step > 0 && (
                  <button onClick={() => setStep(step - 1)} className="flex items-center gap-1 px-4 py-2.5 text-cream/70 hover:text-gold font-montserrat text-sm transition-colors" data-testid="checkout-back">
                ...
                {step < 2 ? (
                  <GoldButton onClick={() => { if (step === 0 && !canProceedStep0) { toast.error(t('checkout.fillRequired')); return; } setStep(step + 1); }} className="px-6" dataTestId="checkout-next">
                    <span className="flex items-center gap-1">{t('checkout.next', 'Next')} <ChevronRight className="w-4 h-4" /></span>
                  </GoldButton>
                ) : (
    ...
    <AuthModal isOpen={showAuthModal} onClose={() => setShowAuthModal(false)}
      onLoginSuccess={() => { setShowAuthModal(false); setStep(0); }} defaultMode={authMode} />
    </React.Fragment>
  );
};
