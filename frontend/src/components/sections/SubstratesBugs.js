...
import { Layers, Bug, ShoppingCart, Eye } from 'lucide-react';
...
const SubstratesBugs = () => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`${API}/wc/products`, {
          params: { category: 'substrates-bugs' }
        });
        setProducts(response.data || []);
      } catch (error) {
        console.error('Error fetching substrates & bugs:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const tabs = [
    ...
    { id: 'bugs', label: 'Springtails & Bugs', icon: Bug },
  ];
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-cream/80 font-montserrat text-sm sm:text-base lg:text-lg max-w-2xl mx-auto">
            Premium substrates, drainage materials, and beneficial springtails. 
            Everything you need to create a thriving, self-sustaining terrarium ecosystem.
          </p>
        </motion.div>
        ...
        {loading ? (
          ...
          >
            ...
            <p className="text-cream/60 font-montserrat text-lg">
              Substrates & Bugs coming soon! Check back later for our selection.
            </p>
          </motion.div>
        )}
      </div>
