...
import { FlaskConical, ShoppingCart, Eye } from 'lucide-react';
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`${API}/wc/products`, {
          params: { category: 'bottles-jars' }
        });
        setProducts(response.data || []);
      } catch (error) {
        console.error('Error fetching bottles & jars:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        >
          <FlaskConical className="w-12 h-12 sm:w-16 sm:h-16 text-gold mx-auto mb-4 sm:mb-6" />
          ...
          <p className="text-cream/80 font-montserrat text-sm sm:text-base lg:text-lg max-w-2xl mx-auto">
            Premium glass containers in various shapes and sizes. 
            Choose the perfect vessel for your miniature ecosystem.
          </p>
        </motion.div>
        ...
        {loading ? (
          ...
          >
            <FlaskConical className="w-16 h-16 text-gold/30 mx-auto mb-4" />
            <p className="text-cream/60 font-montserrat text-lg">
              Bottles & Jars coming soon! Check back later for our selection.
            </p>
          </motion.div>
        )}
      </div>
